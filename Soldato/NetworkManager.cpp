...
void NetworkManager::ProcessSocketData()
{
  ...
  if (bytesReceived > 0)
  {
    ...
    ProcessIncomingMessage(teflonMessage, fromAddr);
  }
...
void NetworkManager::SendAcknowledgment(const std::string& originalMessageId, bool isPositive, const sockaddr_storage& peerAddr)
{
    if (!m_connected.load() || m_socket == INVALID_SOCKET) {
        return;
    }
...
void NetworkManager::ProcessIncomingMessage(const Message& message, const sockaddr_storage& fromAddr)
{
  ...
  if (message.isAcknowledgment()) {
    ...
    if (checksumValid) {
      ...
      SendAcknowledgment(message.messageId, true, fromAddr); // Use the 3-argument version
      ...
      SendAcknowledgment(message.messageId, false, fromAddr); // Use the 3-argument version
